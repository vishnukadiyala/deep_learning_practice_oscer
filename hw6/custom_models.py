...
from keras.layers import GRU, LSTM, MultiHeadAttention

#   Create parser
args = argparse.ArgumentParser()

def create_mha_network(
                            n_tokens = 1000,
                            len_max = 100,
                            n_embeddings = 25,
                            n_mha = None,
    ...
    input_layer = Input(shape=(len_max,), name='input_layer')
    
    x = Embedding(n_tokens, n_embeddings, input_length = len_max)(input_layer)
    
    # Add a Convolutional layer (optional)
    ...
    if conv_size is not None:
        x = AveragePooling1D(
                    pool_size = conv_size[0],
                    name = 'avg_pooling_layer_pre_process'
                    )(x)
    
    for i,n in enumerate(n_mha):
        # We are adding the multi head attention layers
        
        # We are using the same value for Key , Q and V. Self attention.
        layer = MultiHeadAttention(
            num_heads = n,
            key_dim = n,
            dropout = dropout,
            name = 'multi_head_attention_layer_{}'.format(i+1)
        )
    ...
    # Add a Flatten layer to flatten the output of the Multi Head Attention layer

...
class Core50:
    ...
    def create_training_validation_testing_from_datasets(self, rotation:int,
                                                                 ds_folds:[tf.data.Dataset],
                                                                 ntraining:int,
                                                                 seed:int=42,
                                                                 batch_size:int=None,
                                                                 prefetch:int=2,
                                                                 repeat:bool=False,
                                                                 shuffle:int=0)->[tf.data.Dataset]:

        '''
        Translate a list of DataSets describing folds into Datasets that represent the training,
        validation and testing sets, respectively

        :param rotation: Integer rotation (0 <= rot < nobjects)
        :param ds_folds: array of folds described as Datasets
        :param ntraining: Number of training folds
           If ntraining <= # folds-2, then we will have both validation and test sets
           If ntraining == # folds-1, then we will have only a validation set
        :param seed: Random seed for shuffling the data
        ...
        :return: Datasets for each of the training, validation and testing sets
        '''
        ...
        print("Validation: ", folds_validation)
        print("Testing: ", folds_testing)
        print("len: ", len(ds_folds))

        # Assign the training/validation/testing data sets
        ds_training = Core50.repeat_shuffle_batch_prefetch(tf.data.Dataset.sample_from_datasets([ds_folds[i] for i in folds_training]),
                                                    repeat=repeat,
                                                    shuffle=shuffle,
                                                    batch_size=batch_size,
                                                    prefetch=prefetch)
                                                                                         
        ds_validation = Core50.repeat_shuffle_batch_prefetch(ds_folds[folds_validation],
                                                      repeat=repeat,
                                                      shuffle=shuffle,
                                                      batch_size=batch_size,
                                                      prefetch=prefetch)
        if folds_testing is not None:
            ds_testing = Core50.repeat_shuffle_batch_prefetch(ds_folds[folds_testing],
                                                       repeat=repeat,
                                                       shuffle=shuffle,
                                                       batch_size=batch_size,
                                                       prefetch=prefetch)
    ...
    @staticmethod
    def repeat_shuffle_batch_prefetch(ds:tf.data.Dataset,
                                      repeat: bool =False,
                                      shuffle: int =0,
                                      batch_size: int=8,
                                      prefetch: int=2)->tf.data.Dataset:

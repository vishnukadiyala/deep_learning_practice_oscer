...
def exp_type_to_hyperparameters(args):
    ...
    if args.exp_type == 'bmi':
        ...
        p = {'Ntraining': [1,2,3,4,5,9,13,18], 
             'rotation': range(20),
             #'L1_regularization': [0.1, 0.01, 0.001, 0.0001],
             }
    else: 
...
def deep_network_basic(in_shape, n_hidden, out_shape, metrics_in, args):
    ...
    for i,n in enumerate(n_hidden):
        # Add regularization if given a L1 regularization input 
        if args.L1_regularization is not None:
            model.add(Dense(n, use_bias = True, name = "hidden_"+str(i),kernel_regularizer = keras.regularizers.L1(args.L1_regularization) ,activation = args.activation_hidden))
        
        # Add a dense layer otherwise 
        else:
            model.add(Dense(n, use_bias = True, name = "hidden_"+str(i), activation = args.activation_hidden))
            
            # Add a dropout layer if given in the parameter

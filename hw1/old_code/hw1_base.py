...
def exp_type_to_hyperparameters(args):
    ...
    if args.exp_type == 'bmi':
        # HW 1
        p = cartesian_product({'Ntraining': [1,2,3,4,5,9,13,18], 
             'rotation': range(20)})
...
def deep_network_basic(input_shapes, n_hidden, output_shapes, activation_hidden, activation_out,lrate,metrics_in):
    
    
    model = keras.Sequential();
    model.add(InputLayer(input_shape = input_shapes))
    for i,h in enumerate(n_hidden):
        model.add(Dense(h, use_bias = True , name = "hidden_%d"%(i), activation = activation_hidden))
    
    model.add(Dense(output_shapes,  use_bias = True, name = "output", activation = activation_out))

    opt = tf.keras.optimizers.Adam(learning_rate=lrate, beta_1=0.9, beta_2=0.999,
                                epsilon=None, decay=0.0, amsgrad=False)
    

    model.compile(loss = metrics_in, optimizer = opt)
...
def execute_exp(args=None):
    ...
    model = deep_network_basic(ins.shape[1], tuple(args.hidden), outs.shape[1],
                               args.activation_hidden, args.activation_out, args.lrate,
                               metrics_in=[fvaf, rmse])
    #model = keras.Sequential(ins.shape[1], tuple(args.hidden), outs.shape[1], metrics=[fvaf, rmse])
